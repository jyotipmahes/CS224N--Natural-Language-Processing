...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x_sum = np.sqrt(np.sum(x**2, 1))  #sum of square row wise
    x /= np.reshape(x_sum, (-1, 1)) + 1e-20 #diving each element by sum of square matrix,reshape to make it column matrix
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    N = outputVectors.shape[0]    #to get total no. of words in vocab                 
    y = np.zeros(N)   # one hot vector representation for target word
    y[target] = 1     #y is 1*N dim vector with 1 for target word and rest 0
    
    score = np.dot(predicted, outputVectors.T) #dot product for weights and word           
    out = softmax(score)  # Applying softmax on score
    
    cost = np.sum(-y * np.log(out))  #Cross entropy cost  
    
    dout = out - y   # 1*N vector (ynat-y)         
    gradPred = np.dot(dout, outputVectors) # 1*d dimension vector,grad for pred word            
    grad = np.dot(dout.T, predicted) # N*d vector for all words         
    ### END YOUR CODE
...
def getNegativeSamples(target, dataset, K):
    ...
    indices = [None] * K  #seting K empty indices in a matrix
    for k in range(K):
        newidx = dataset.sampleTokenIdx()
        while newidx == target:
            newidx = dataset.sampleTokenIdx() #ensuring target index is not selected
        indices[k] = newidx
    return indices
